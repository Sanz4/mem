...
simbol = "⚬"
petik = '```'
fake = 'Copyright\ ©Sanz'//GANTI NAMA KAMU BEP
ban =[]
tp = "```"
// Database
...
module.exports = dha = async (dha, mek) => {
    ...
    //    const isPremium = premium.checkPremiumUser(sender, _premium)
        ...
        const listmsg = (from, title, desc, list) => { // ngeread nya pake rowsId, jadi command nya ga keliatan
            let po = dha.prepareMessageFromContent(from, {"listMessage": {"title": title,"description": desc,"buttonText": "Pilih Disini","footerText": "Copyright By ©Sanz","listType": "SINGLE_SELECT","sections": list}}, {})
...
dha.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})		    
button = [
...
await dha.sendMessage(from, buttons, MessageType.buttonsMessage, {quoted: mek, contextInfo: { mentionedJid: [sender, `${owner}@s.whatsapp.net`]}})
}
...
if (isGroup && !isGroupAdmins && isBotGroupAdmins) {
                if (isImage) {
                        var filePath = await dha.downloadAndSaveMediaMessage(mek);
...
if (!isGroup && !isCmd && !mek.key.fromMe && !autovn) {
             for (let i of totalchat) {
             	autovn = false
         dha.updatePresence(i.jid, Presence.recording)
                   }
             }
//Auto Ngetik
if (!isGroup && !isCmd && !mek.key.fromMe && !autongetik) {
             for (let i of totalchat) {
             	autongetik = false
          dha.updatePresence(i.jid, Presence.composing)
                   }
             }
let {
    banChats,
...
kma = gam1
kntl = await dha.prepareMessage(from, kma, location)
...
${petik}❏ Wa Version: ${dha.user.phone.wa_version}${petik}
${petik}❏ Os Version: ${dha.user.phone.os_version}${petik}
${petik}❏ Device Manufacturer: ${dha.user.phone.device_manufacturer}${petik}
${petik}❏ Device Model: ${dha.user.phone.device_model}${petik}
${petik}❏ Os Build Number: ${dha.user.phone.os_build_number}${petik}
...
©Sanz`
   dha.sendMessage(from, {"contentText": textnya ,"footerText": `${fake}`, "buttons": [
  {buttonId: `${prefix}update`, buttonText:{displayText: '/<DASBOARD'}, type: 1},
...
externalAdReply: {
            ...
            "mediaType": 2,
            "thumbnail": thumb2,
            "mediaUrl": `https://www.instagram.com/p/CVSK_y8IaUZ/?utm_medium=copy_link`
        }
    }, quoted: floc2}) 
    break
case 'setreply':
...
case 'setthumb':
	        if ((isMedia && !mek.message.videoMessage || isQuotedImage || isQuotedSticker) && args.length == 0) {
          	boij = isQuotedImage || isQuotedSticker ? JSON.parse(JSON.stringify(dha).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
...
case 'tiktokdl':
      case 'tiktok':
                ...
                reply(mess.wait)
                link = args.join(' ')
                dppa = await fetchJson(`https://api.lolhuman.xyz/api/tiktok?apikey=${setting.lolkey}&url=${link}`)
                uhyy = dppa.result
...
dha.sendMessage(from, khs, audio, {quoted:mek, mimetype:'audio/mp4', filename:'tiktok.mp3'})
})
break
      case 'fb':
      case 'facebook':
...
*「 Dinn Bot 」*`
                getBuffer(res[0].posterImage.original).then(ress => {
                	dha.sendMessage(from, ress, image, {quoted: mek, caption: teks})
                    })
                })
                } catch (e) {
    ...
    case 'kusonime':
        ...
        await axios.get(link1).then(async res => {
            ...
            return await dha.sendMessage(from, `Judul : ${judul}\n${genre}\n${totaleps}\n${durasi}\n${tglrilis}\n\nResolusi 360p : \n${result360}\n\nResolusi 480p : \n${result480}\n\nResolusi 720p : \n${result720}\n\nResolusi 1080p : \n${result1080}`, text , {quoted: mek})
   })
            })
                      } catch (err) {
...
break
    ...
    case 'quotesyt':
              ...
              let xixi = await dha.prepareMessageMedia(dapuhy, image, {thumbnail: thumb})
          let but = [
          ...
          dha.sendMessage(from, buttonMessages, MessageType.buttonsMessage, {quoted: mek, thumbnail: thumb})
...
limitAdd(sender, limit)
        break;    
                ...
                yang = await getBuffer(`https://api.lolhuman.xyz/api/random/${command}?apikey=${lolkey}`)
                              let yag = await dha.prepareMessageMedia(yang, image, {thumbnail: thumb})
          let ge = [
          ...
          dha.sendMessage(from, gb, MessageType.buttonsMessage, {quoted: mek, thumbnail: thumb})
    break
...
kntl = await getBuffer(`https://api.dapuhy.ga/api/anime/storyanime?apikey=${dappakey}`)                    
              let xixiu = await dha.prepareMessageMedia(kntl, video, {thumbnail: thumb})
          let bot = [
            {buttonId: `${prefix+command}`, buttonText: {displayText: 'Lagi'}, type: 1}
          ]
          let buttonMessagees = {
          ...
          dha.sendMessage(from, buttonMessagees, MessageType.buttonsMessage, {quoted: mek, thumbnail: thumb})
    break
...
mhan2 = await dha.prepareMessage(from, kntl, video, { thumbnail: Mfake })
ggbutsan = [
...
await dha.sendMessage(from, gbbuttonan, MessageType.buttonsMessage, {
...
case 'nsfwloli':
                    ...
                    imageMsg = ( await dha.prepareMessage(from, anu, 'imageMessage', {thumbnail: Buffer.alloc(0)})).message.imageMessage
                    ...
                    prep = await dha.prepareMessageFromContent(from,{buttonsMessage},{quoted: mek, contextInfo:{"externalAdReply": {"title": `Nih Kak ${pushname}`, mediaType: 2, "thumbnailUrl": "https://telegra.ph/file/6b0259fd741e108910fbe.jpg","previewType": "VIDEO","mediaUrl": `https://youtu.be/5odMRQDrhoI`}}})
                    dha.relayWAMessage(prep)         
                    limitAdd(sender, limit)           
					break
case 'nsfwyuri':
...
case 'play':
				case 'ytplay':
    case 'ytdl':
          		if (args.length === 0) return reply(`Kirim perintah *${prefix}play* _Judul lagu yang akan dicari_ *Atau,* _Link_`)
...
*_Harap tunggu sebentar*` 
thumb = res.all[0].image
ya = await getBuffer(thumb) 
sendButLocation(from, thumbInfo, `${fake}\n @0`, {jpegThumbnail:ya}, [{buttonId:`${prefix}playy ${q}`,buttonText:{displayText:'🎵AUDIO'},type:1},{buttonId:`${prefix}play2 ${q}`,buttonText:{displayText:'🎥VIDEO'},type:1}], {contextInfo: { mentionedJid: [sender, '0@s.whatsapp.net']}})
break
}
...
`
      ...
      case 'pinterest':
            ...
            let di = await getBuffer(ac)
                          let xixii = await dha.prepareMessageMedia(dapuhy, image, {thumbnail: thumb})
          let buut = [
          ...
          dha.sendMessage(from, buttonMessagese, MessageType.buttonsMessage, {quoted: mek, thumbnail: thumb})
   ...
   if (args.length == 0) return reply(`Kirim perintah *${prefix}shopee [ query ]*\nContoh : ${prefix}anime `)
					try {
						wall.getAnimeWall2(`${body.slice(11)}`).then(res => {
							getBuffer(res[0].image).then(ress => dha.sendMessage(from, ress, image, {quoted: mek, caption: `${res[0].title}`}))
						})
					} catch (e) {
...
dha.sendMessage(from, tapib1, document, { quoted: mek, filename:`${botname} ~ 404 ${vipi}.mp3`, mimetype: 'audio/application' })
